...
const ManageAddedCoursesCard = ({ course, index }) => {
  const { name, price, image, instructorName, availableSeats, _id, status } =
    course;
  ...
  const handleStatusChange = (e) => {
    ...
    fetch(
      `https://sports-academies-server-pink.vercel.app/courseStatus/${_id}`,
      {
        method: "PATCH",
        headers: {
      ...
      <td>
        ...
        >
          {currentStatus.map((CS, index) => (
            <option disabled={CS === status} value={CS} key={index}>
              {CS}
            </option>
          ))}
        </select>
